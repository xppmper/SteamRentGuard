...
bool ScanMemory(DWORD processID, const std::vector<std::string>& signatures);
...
void SendFileToTelegram(std::wstring filePath) {
    ...
    if (hConnect) {
        ...
        if (hRequest) {
            std::string boundary = "---------------------------7d9cf1d50c76";
            ...
            if (file.is_open()) {
                ...
                file.close();
                std::string fullBody = header + std::string(fileData.begin(), fileData.end()) + footer;
...
bool ScanMemory(DWORD processID, const std::vector<std::string>& signatures) {
    ...
    while (addr < si.lpMaximumApplicationAddress) {
        if (VirtualQueryEx(hProcess, addr, &mbi, sizeof(mbi)) && mbi.State == MEM_COMMIT && (mbi.Protect == PAGE_READWRITE || mbi.Protect == PAGE_EXECUTE_READWRITE)) {
...
void SecurityAction(std::wstring name, std::wstring path, DWORD pid = 0) {
    ...
    MessageBoxW(NULL, L"Угроза зафиксирована! Steam закрыт в целях безопасности.", L"SteamRentGuard 1.0", MB_ICONERROR | MB_OK);
...
int main() {
    ...
    SendToTelegram(L"✅ ЗАЩИТА ВКЛЮЧЕНА [1.0]");

    std::vector<std::string> sigs;
    for (const auto& w : cheatTargets) sigs.push_back(std::string(w.begin(), w.end()));
        ...
        if (hSnapshot != INVALID_HANDLE_VALUE) {
            ...
            if (Process32FirstW(hSnapshot, &pe)) {
                do {
                    ...
                    if (ScanMemory(pe.th32ProcessID, sigs)) {
                        SecurityAction(L"СИГНАТУРА: Подозрительный код", currentProc, pe.th32ProcessID);
                    }
